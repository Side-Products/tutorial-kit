...
const Reveal = ({ children, delay = 0, dur = 20, lift = 16, blur = 12 }) => {
	...
	const t = interpolate(frame, [delay, delay + dur], [0, 1], { easing: EASE, extrapolateLeft: "clamp", extrapolateRight: "clamp" });
	return (
		<div
			style={{
				opacity: t,
...
const Rule = ({ delay = 0, width = 220, color = "rgba(255,255,255,0.22)" }) => {
	const frame = useCurrentFrame();
	const w = interpolate(frame, [delay, delay + 26], [0, width], { easing: EASE, extrapolateLeft: "clamp", extrapolateRight: "clamp" });
...
export const IntroCard = ({ timeline }) => {
	...
	const exit = interpolate(frame, [total - 9, total - 1], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
	return (
		<AbsoluteFill
			style={{
				alignItems: "center",
		...
		>
			<div style={{ textAlign: "center", maxWidth: "68%" }}>
				<Reveal delay={0} lift={10} blur={8}>
					{timeline.brand?.logo ? (
						<Img src={staticFile(timeline.brand.logo)} style={{ height: 78, display: "block", margin: "0 auto" }} />
					) : (
						<div style={{ fontSize: 30, fontWeight: 500, letterSpacing: 5, textTransform: "uppercase", color: "rgba(255,255,255,0.45)" }}>
							{timeline.brand?.name || ""}
						</div>
					)}
				</Reveal>
				...
				<Reveal delay={5}>
					<div style={{ fontSize: 108, fontWeight: 600, lineHeight: 1.12, color: colors.text || "#fff", letterSpacing: -1.5 }}>
						{timeline.intro.title}
					</div>
				</Reveal>
				{timeline.intro.subtitle ? (
					<Reveal delay={12} lift={12} blur={8}>
						<div style={{ marginTop: 34, fontSize: 42, fontWeight: 400, color: "rgba(255,255,255,0.5)" }}>{timeline.intro.subtitle}</div>
					</Reveal>
				) : null}
			</div>
		</AbsoluteFill>
...
export const OutroCard = ({ timeline }) => {
	...
	const settle = interpolate(frame, [0, Math.max(24, total)], [1.04, 1], { easing: EASE, extrapolateRight: "clamp" });
	const fade = interpolate(frame, [total - 8, total - 1], [1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
	return (
		<AbsoluteFill style={{ alignItems: "center", justifyContent: "center", opacity: fade, transform: `scale(${settle})` }}>
			<div style={{ textAlign: "center" }}>
				<Reveal delay={0} lift={12} blur={10}>
					{timeline.brand?.logo ? (
						<Img src={staticFile(timeline.brand.logo)} style={{ height: 138, display: "block", margin: "0 auto" }} />
					) : (
						<div style={{ fontSize: 130, fontWeight: 700, color: colors.text || "#fff", letterSpacing: -1.5 }}>{timeline.outro.title}</div>
					)}
				</Reveal>
